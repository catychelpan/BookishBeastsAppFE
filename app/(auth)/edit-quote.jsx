...
const EditQuote = () => {
    ...
    const { bookId, quoteId } = useLocalSearchParams();
    ...
    const fetchQuote = async () => {
        const { data } = await axios.get(`books/${bookId}/quote/${quoteId}`);
        setText(data.text);
        const mappedNotes = data?.notes?.map(item => ({
            id: item.id,
            createdAt: item.createdAt,
            text: item.content,
            noteTypeName: item.typeName,
            noteTypeColor: item.color,
            noteTypeIcon: item.icon
        }))
        setQuote(prev => ({...prev, notes: mappedNotes,
    ...
    const handleSaveQuote = async () => {
        try {
            await axios.put(`books/${bookId}/quote`, {
                id: quoteId,
                content: text,
                noteIds: getNoteIds(),
    ...
    const handleRemoveRepetitionGroup = (collectionId) => {
    ...
    return <SafeAreaView className="bg-[#F7F7F7] h-full">
            <View className="max-h-[60px] justify-between items-center flex-row h-full mx-5 mb-7">
                <View className="flex-row items-center mt-2">
                    ...
                    <Text className="text-black font-cygrebold text-[22px] font-bold">Edit Quote</Text>
                </View>
                <TouchableOpacity
                            ...
                            <View className="flex-wrap flex-row justify-start self-start flex-1">
                                { quote?.repetitionGroups?.map(item =>
                                    <Collection
                                        ...
                                        name={item.name}
                                        showCloseBtn={true}
                                        handleRemove={handleRemoveRepetitionGroup}
                                    />
                                ) }
                            </View>
                            <TouchableOpacity
                    ...
                    <ScrollView 
                        ...
                        horizontal>
                            <View className="flex-1">
                                <TouchableOpacity
                                    onPress={() => router.push({pathname: 'select-notes', params: { id: bookId }})}

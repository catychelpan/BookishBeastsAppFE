...
const SelectNoteCollections = () => {
    ...
    const fetchCollections = useCallback(debounce(async (text) => {
        try {
            ...
            const { data } = await axios.get(`users/note-collections?${queryParams.toString()}`);
...
const NewCollection = ({ containerStyles }) => {
    ...
    return <View className={`bg-black rounded-[20px] justify-between p-4 max-w-[169px] mb-4 h-[174px] ${containerStyles}`}>
        <Text className="font-cygrebold mb-4 text-[22px] leading-[26.4px] font-bold text-[#ffffff]" numberOfLines={2} ellipsizeMode="tail">New Collection</Text>
        <TouchableOpacity
